...
export class AgentDecisionController {
    static async list(req: Request, res: Response): Promise<any> {
        try {
            ...
            console.error("[AgentDecision List Error]: ", error.message);
            return res.status(500).json({ message: "Failed to fetch agent decisions." })
        }
    }
    static async stats(req: Request, res: Response) {
        try {
            ...
            console.error("[AgentDecision Stats Error]:", error.message);

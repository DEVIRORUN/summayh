  ...
  static async PaystackThread(req: Request, res: Response) {
    try {
      ...
      if (!rawBody) {
        console.error(
          new Date(),
          "-> [Webhook]: req.rawBody is missing - check expess.json() verify cllback in index.ts",
        );
      ...
      if (!isValid) {
        console.warn(
          new Date(),
          "-> [Security Warning]: Unauthorized webhook signatue mismatch dropped.",
        );
      ...
      if (event === "charge.success") {
        ...
        if (metadata?.founderPassPurchaseId) {
          ...
          console.error(
            `[Webhook Error]: No recognizable identifiew in metadata for referece: ${data.reference}`,
          );
        }
      } else if (event === "transfer.success") {
        await PaymentService.handleTransferSuccess(data);
      } else if (event === "transfer.failed") {
        await PaymentService.handleTransferFailed(data);
      } else if (event === "transfer.reversed") {
        await PaymentService.handleTransferReversed(data);
      } else {

import { prisma } from "../utils/prisma"; // adjust to however you import your Prisma client elsewhere
import { PaystackService } from "./paystack.service";

export class FoundersPassService {
  /**
   * Returns the current active Founders Pass configuration (price, cap, status).
   * Assumes a single active config row exists.
   */
  static async getActiveConfig() {
    const config = await prisma.foundersPassConfig.findFirst({
      where: { isActive: true },
  ...
  static async getFoundersPassAvailability() {
    ...
    const soldCount = await prisma.foundersPassPurchase.count({
   ...
   */
  static async initializeFoundersPassPayment(
    sellerId: string,
    buyerEmail: string,
  ): Promise<any> {
    try {
      ...
      const existing = await prisma.foundersPassPurchase.findFirst({
        where: { sellerId },
      });
      ...
      if (!purchase) {
        purchase = await prisma.foundersPassPurchase.create({
          data: {
            sellerId,
      ...
      const paymentInit = await PaystackService.initializeTransaction(
        buyerEmail,
        config.priceNaira,
        { founderPassPurchaseId: purchase.id }, // reused as metadata.orderId equivalent — see note below
      );

      await prisma.foundersPassPurchase.update({
   ...
   */
  static async activateFoundersPass(reference: string): Promise<any> {
    try {
      return await prisma.$transaction(async (tx) => {
        ...
        if (!purchase) {
          throw new Error(
            `No Founders Pass purchase found for reference: ${reference}`,
          );
        }

        if (purchase.status === "SUCCESS") {
          console.log(
            `[Webhook Info]: Founders Pass ${purchase.id} already activated.`,
          );
        ...
        console.log(
          `[Webhook Success]: Founders Pass activated for seller ${purchase.sellerId}`,
        );
        return { success: true };
      });
    } catch (error) {
      throw new Error(
        "[Webhook Failure]: Founders Pass was not ACTIVATED",
      );
    }
  }
}

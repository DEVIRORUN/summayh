...
export class InvoiceService {
   ...
   */
  static async initializeInvoicePayment(
    invoiceId: string,
    buyerEmail: string,
    buyerName: string,
  ): Promise<any> {
  ...
  static async activateInvoice(reference: string) {
    return await prisma.$transaction(async (tx) => {
      ...
      if (invoice.status === "PAID") {
        console.log(
          `[Webhook Info]: Invoice ${invoice.id} already marked as PAID.`,
        );
      ...
      console.log(
        `[Webhook Success]: Invoice ${invoice.id} paid and released to seller ${invoice.sellerId}`,
      );
      return { success: true };
    });
  }

...
export class CallService {
    ...
    static async flagNoShow(bookingId: string) {
        const booking = await prisma.sessionBooking.findUniqueOrThrow({
            where: { id: bookingId },
            include: {
        ...
        if (!booking.callSession || booking.callSession.status === "ACTIVE") return;

        const joinedUserIds = new Set(
            booking.callSession.events.filter(e => e.type === "JOINED").map(e => (e.metadata as any)?.userId)
        )

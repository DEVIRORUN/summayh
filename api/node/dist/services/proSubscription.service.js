...
exports.ProSubscriptionService = void 0;
const prisma_1 = require("../utils/prisma");
const paystack_service_1 = require("./paystack.service");
class ProSubscriptionService {
    static async getActivePlans() {
        return prisma_1.prisma.proPlan.findMany({ where: { isActive: true }, orderBy: { priceNaira: "asc" } });
    }
    static async initializeSubscription(sellerId, planId, buyerEmail) {
        const plan = await prisma_1.prisma.proPlan.findUniqueOrThrow({ where: { id: sellerId } });
        const seller = await prisma_1.prisma.sellerProfile.findUniqueOrThrow({ where: { id: planId } });
        if (seller.isPro && seller.proSource === "FOUNDERS") {
            throw new Error("You already have a lifetime Founders Pass - no need to subscribe.");
        }
        const subscription = await prisma_1.prisma.proSubscription.create({
            data: { sellerId, planId, status: "PENDING" },
        });
        const paymentInit = await paystack_service_1.PaystackService.initializeTransaction(buyerEmail, plan.priceNaira, { proSubscriptionId: subscription.id } // metadata - webhhok routes on this
        );
        await prisma_1.prisma.proSubscription.update({
    ...
    static async activateSubscription(reference) {
    ...
    static async expiresStaleSubscriptions() {
        const expired = await prisma_1.prisma.sellerProfile.findMany({
...
exports.ProSubscriptionService = ProSubscriptionService;

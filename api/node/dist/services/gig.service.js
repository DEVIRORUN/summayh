...
class GigService {
     ...
     */
    static async createDraftGig(title, categoryId, tags, userId, deliveryMode = "DIGITAL") {
        try {
            ...
            const draftGig = await prisma_1.prisma.gig.create({
                data: {
                    title,
                    tags,
                    state: "DRAFT",
    ...
    static async addDescToDraft(gigId, description, faqs, sellerId) {
        try {
            console.log(new Date(), "-> [ADD DESC TO DRAFT]: Hit!!!");
            // We get the existing Gig
            const descGig = await prisma_1.prisma.$transaction(async (tx) => {
    ...
    static async addTiersToGig(gigId, tiers, sellerId) {
        try {
            console.log(new Date(), "-> [ADD TIERS TO DRAFT]: Hit!!!");
            const tierGig = await prisma_1.prisma.$transaction(async (tx) => {
                const newGig = await tx.gig.findFirst({
                    where: { id: gigId, sellerId },
                });
                if (!newGig) {
                    throw new Error("Gig not found or you don't have permissino to edit it.");
                }
                await tx.gigTier.deleteMany({ where: { gigId } });
                const isLive = newGig.deliveryMode === "LIVE";
                const buildTierData = (label, tier) => ({
                    gigId,
                    label,
                ...
                await tx.gigTier.createMany({
                    data: [
                        buildTierData(prisma_2.TierLabel.BASIC, tiers.basic),
                        buildTierData(prisma_2.TierLabel.STANDARD, tiers.standard),
                        buildTierData(prisma_2.TierLabel.PREMIUM, tiers.premium),
                    ],
                });
    ...
    static async generateUploadUrl(gigId, sellerId, fileType, slot) {
        ...
        if (!gig) {
            throw new Error("Gig not found or you don't have access/permissoon to edit this gig.");
        }
        // Now we build a unique key (path) for this file inside the bucket
        const extension = fileType.split("/")[1]; // "png", "mp4"
        const key = `gigs/${gigId}/${slot}-${(0, crypto_1.randomUUID)()}.${extension}`;
    ...
    static async saveGalleryToGig(gigId, sellerId, images, // 1-3 URLs
    video) {
        try {
    ...
    static async publishGig(gigId, sellerId) {
        try {
            // Needs transactin here for sure
            const publishGig = await prisma_1.prisma.$transaction(async (tx) => {
                ...
                if (!gig) {
                    throw new Error("Gig not found or you are not allowed ot edit this Gig");
                }
                if (!gig?.description)
                ...
                if (isLive) {
                    const invalidTier = gig.tiers.find((t) => !t.sessionLengthMin || !t.totalSessions);
                    if (invalidTier) {
    ...
    static async addQuestionsToGig(gigId, sellerId, requirementTemplates) {
        try {
            console.log(new Date(), "-> [ADD QUESTIONS TO DRAFT]: Hit!!!");
            const questionGig = await prisma_1.prisma.$transaction(async (tx) => {
                const newGig = await tx.gig.findFirst({
                    where: { id: gigId, sellerId },
                });
                if (!newGig) {
                    throw new Error("Gig not found, You don't have permission to edit it.");
                }
            ...
            console.log(new Date(), "-> [ADD QUESTIONS TO DRAFT]: Draft Gig created!!!");
            return questionGig;
        }
        catch (err) {
            throw err;
    ...
    //   }
    // }
    static async readGigData(userId, gigId) {
        try {
            console.log(new Date(), "-> [Gig Service read]: Hit!");
            const mainGig = await prisma_1.prisma.gig.findUnique({
                where: { id: gigId },
                include: {
                    category: true,
                    tiers: {
    ...
    //   }
    // }
    static async updateGigData(gigId, userId, title, description, tags, categoryId, tiers) {
        try {
            console.log("DEBUG LOOKUP -> gigId:", gigId, "userId:", userId);
            // EXTRA validation
            const sellerProfile = await prisma_1.prisma.sellerProfile.findFirst({
                where: { userId: userId },
            });
            if (!sellerProfile) {
            ...
            return await prisma_1.prisma.$transaction(async (tx) => {
                ...
                if (tiers !== undefined && tiers !== null) {
                    for (const [labelKey, tierData] of Object.entries(tiers)) {
                        if (!tierData)
                            continue;
                        const tierLabel = labelKey.toUpperCase();
    ...
    static async deleteGig(userId, gigId) {
        try {
            ...
            return await prisma_1.prisma.gig.update({
                where: { id: gigId },
                data: { state: "INACTIVE" },
            });
        }
    ...
    static async listGigs(userId, page = 1, limit = 30, categoryId) {
        try {
            const skip = (page - 1) * limit;
            // basic filter (only ACTIVE GIGS)
            const whereClause = { state: "ACTIVE" };
            if (categoryId) {
            ...
            const totalGigs = await prisma_1.prisma.gig.count({ where: whereClause });
    ...
    static async getAllGigsBySeller(userId, page = 1, limit = 15) {
        try {
            ...
            const totalGigs = await prisma_1.prisma.gig.count({
                where: { seller: { userId } },
            });
    ...
    static async addBulkingPricing(gigId, tierId, userId, bands) {
        try {
            ...
            for (const band of bands) {
                ...
                if (band.totalPrice >= unitTotal) {
                    throw new Error(`Quantity ${band.quantity}: totalPrice (₦${band.totalPrice}) must be less than ` +
            ...
            const results = await prisma_1.prisma.$transaction(bands.map((band) => prisma_1.prisma.tierQuantityPrice.upsert({
                where: {
                    gigTierId_quantity: {
                        gigTierId: tierId,
                ...
                create: {
                    ...
                    discountPercentage: Math.round(((tier.price * band.quantity - band.totalPrice) / tier.price) *
                        band.quantity),
                },
            })));
            return {
    ...
    static async getBulkPricing(gigId, tierId) {
        try {
            const tier = await prisma_1.prisma.gigTier.findUnique({ where: { id: tierId } });
            if (!tier) {
                console.error(`Tier with id ${tierId} not found for gig ${gigId}`);
            }
            const bulkPricing = await prisma_1.prisma.tierQuantityPrice.findMany({

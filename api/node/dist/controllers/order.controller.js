...
class OrderController {
    ...
    static async deliverOrder(req, res) {
        try {
            ...
            const sellerId = req.user.id; // Tailor this to your auth token extraction
            const updatedOrder = await order_service_1.OrderService.submitOrderDelivery(orderId, sellerId);
    ...
    static async acceptDelivery(req, res) {
        try {
            ...
            const { completedOrder } = await order_service_1.OrderService.acceptOrderDelivery(orderId, buyerId);
            // Then, query teh db for notification params to ensure relatiosn exist
            const detailedOrder = await prisma_1.prisma.order.findUnique({
                where: { id: orderId },
                include: {
                    buyer: true,
            ...
            if (buyerPhone) {
                termii_service_1.TermiiService.notifyOrderCompleted(buyerPhone, detailedOrder.gig.title).catch(err => {
            ...
            if (sellerPhone) {
                ...
                termii_service_1.TermiiService.notifyPayoutSent(sellerPhone, payoutAmountInNaira).catch(err => {
    ...
    static async requestRevision(req, res) {
        try {
            const { id: orderId } = req.params;
            const buyerId = req.userId;
            const revisedOrder = await order_service_1.OrderService.requestOrderRevisison(orderId, buyerId);
            return res.status(200).json({
    ...
    static async createOrder(req, res) {
        try {
            ...
            const buyer = await prisma_1.prisma.user.findUnique({ where: { id: buyerId }, select: { id: true, email: true, phoneNumber: true } });
            if (!buyer)
                return res.status(404).json({ message: "Buyer not found." });
            // Extract targeted tier configuration chosen by the user
            const matchedTier = gig.tiers.find((t) => t.label.toUpperCase() === selectedTierLabel.toUpperCase());
            if (!matchedTier) {
            ...
            if (quantity > 1) {
                const bulkBand = await prisma_1.prisma.tierQuantityPrice.findUnique({
                    where: {
                        gigTierId_quantity: {
            ...
            if (buyer.phoneNumber) {
                termii_service_1.TermiiService.notifyOrderPlaced(buyer.phoneNumber, gig.title).catch(err => console.error("Failed to notify buyer of order placement:", err));
            }
            ...
            const paymentInitialize = await paystack_service_1.PaystackService.initializeTransaction(buyerEmail || buyer.email, finalPrice, // Paystack operate in Kobo
            {
                orderId: newOrder.id,
    ...
    static async cancelOrder(req, res) {
        try {
            ...
            if (!reason)
                return res.status(400).json({ message: "Please input the Reason you wnat to cancel this order." });
            const cancel = await order_service_1.OrderService.cancelOrder(orderId, userId, reason);
            return res.status(200).json({
    ...
    static async getOrder(req, res) {
        try {
            ...
            const { orderId } = req.params;
            if (!orderId)
                return res.status(400).json({ messsage: "No correct OrderId was Inputed In, bro." });
            const userId = req.userId;
            const order = await order_service_1.OrderService.getOrder(orderId, userId);
    ...
    static async getOrderAsBuyer(req, res) {
        try {
            const userId = req.userId;
            const page = parseInt(req.query.page) || 1;
            const limit = parseInt(req.query.limit) || 20;
            const orderData = await order_service_1.OrderService.getOrdersAsBuyer(userId, page, limit);
            return res.status(200).json({
    ...
    static async getOrderAsSeller(req, res) {
        try {
            const userId = req.userId;
            const page = parseInt(req.query.page) || 1;
            const limit = parseInt(req.query.limit) || 20;
            const orderData = await order_service_1.OrderService.getMyOrderAsSeller(userId, page, limit);
            return res.status(200).json({
                message: "Successfully got the order DATA",
                data: orderData
            });
    ...
    static async verifyOrderByReference(req, res) {
        try {
    ...
    static async submitOrderRequirements(req, res) {
        try {
            const buyerId = req.userId;
            const { orderId } = req.params;
            ...
            const updatedOrder = await order_service_1.OrderService.submitOrderRequirements(orderId, buyerId, answers);
            // Notify the Seller via SMS / WhatsApp that has started ticking
            const buyer = await prisma_1.prisma.user.findUnique({ where: { id: buyerId } });
            if (buyer?.phoneNumber) {
                termii_service_1.TermiiService.notifySellerRequirementsSubmitted(order.sellerId, order.id).catch(err => {
    ...
    static async scheduleNextSession(req, res) {
        try {
            const buyerId = req.userId;
            const { orderId } = req.params;
            ...
            const booking = await order_service_1.OrderService.scheduleNextSession(orderId, buyerId, scheduledStart, scheduledEnd);

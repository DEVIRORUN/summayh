...
async function main() {
    ...
    const seller = await prisma_1.prisma.sellerProfile.findUnique({ where: { id: "66fcb691-167b-42a1-851c-64968ee28053" } });
    const buyer = await prisma_1.prisma.user.findUnique({ where: { id: "d356296a-c27a-4977-8ce0-408961ace407" } });
    if (!seller || !buyer) {
    ...
    let gig = await prisma_1.prisma.gig.findFirst({ where: { sellerId: seller.id } });
    if (!gig) {
        const category = await prisma_1.prisma.category.findFirst();
        if (!category)
            throw new Error("Need at least one Category in DB first");
        gig = await prisma_1.prisma.gig.create({
            data: {
                sellerId: seller.id,
    ...
    let tier = await prisma_1.prisma.gigTier.findFirst({ where: { gigId: gig.id } });
    if (!tier) {
        tier = await prisma_1.prisma.gigTier.create({
            data: {
                gigId: gig.id,
                label: "BASIC",
    ...
    const sessionPackage = await prisma_1.prisma.sessionPackage.create({
        data: {
            orderId: order.id,
            gigTierid: tier.id,
            sessionLengthMin: 30,
